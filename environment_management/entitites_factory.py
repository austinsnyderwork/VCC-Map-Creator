...
def _apply_create_clinic_sites(row, entities_manager: EntitiesManager):
    ...
    if not entities_manager.contains_entity(entities.VccClinicSite, name=origin_clinic_name, city_name=origin_city_name):
        ...
        city.add_clinic_site(origin_clinic, direction='leaving')
    ...
    if not entities_manager.contains_entity(entities.VccClinicSite, name=visiting_clinic_name,
        ...
        city.add_clinic_site(visiting_clinic, direction='visiting')
...
def _apply_create_providers(row, entities_manager: EntitiesManager):
    ...
    visiting_clinic_name = row['visiting_site']
    visiting_city_name = row['visiting_city']
    if not entities_manager.contains_entity(entities.Provider, name=row['consultant_name'], specialty=row['specialty']):
        provider = entities.Provider(name=row['consultant_name'])
        entities_manager.add_entity(provider)
    else:
        provider = entities_manager.get_provider(name=row['consultant'])

    specialty = row['specialty']
    origin_site = entities_manager.get_vcc_clinic_site(name=origin_clinic_name, city_name=origin_city_name)
    visiting_site = entities_manager.get_vcc_clinic_site(name=visiting_clinic_name, city_name=visiting_city_name)
    new_provider_assignment = entities.ProviderAssignment(provider=provider,
                                                          specialty=specialty,
                                                          origin_site=origin_site,
                                                          visiting_site=visiting_site)
    origin_site.add_provider_assignment(new_provider_assignment, direction='leaving')
    visiting_site.add_provider_assignment(new_provider_assignment, direction='visiting')
