...
class ScanPolysManager:
    ...
    def __init__(self):
        ...
        self.scan_polys_by_num_intersections = {}

    def add_scan_poly(self, scan_poly: ScanPoly):
        num_intersections = len(scan_poly.intersecting_polys)
        if num_intersections not in self.scan_polys_by_num_intersections:
            self.scan_polys_by_num_intersections[num_intersections] = []
        self.scan_polys_by_num_intersections[num_intersections].append(scan_poly)
    ...
    def get_least_intersections_poly_groups(self, poly_attributes_to_omit: dict, poly_types_to_omit: list[str]):
        ...
        poly_groups = []
        intersections = sorted(list(self.scan_polys_by_num_intersections.keys()))
        for intersection_num in intersections:
            for scan_poly in self.scan_polys_by_num_intersections[intersection_num]:
            ...
            if met_omit_condition:
                return poly_groups
        if len(self.scan_polys_by_num_intersections) == 0:
            return []

        return self.scan_polys_by_num_intersections[intersections[0]]
