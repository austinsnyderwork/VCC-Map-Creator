...
class RtreeVisualElementsMap:
    ...
    def add_visual_element(self, visual_element: VisualElement):
        ...
        self._rtree.insert(poly_idx, poly.bounds, obj=poly)
    ...
    def determine_nearest(self, query_poly, elements_to_ignore: list) -> dict[VisualElement: float]:
        nearest_ids = list(self._rtree.nearest(query_poly.bounds, 10))
    ...
    def determine_intersecting_visual_elements(self, visual_element: VisualElement):
        indices = list(self._rtree.intersection(visual_element.polygon.bounds))
